...
void aesDecryptBlock(
  ...
  var r0 =
      ((0xFF & sInv[0xFF & (v0 >> 24)]) << 24) | ((0xFF & sInv[0xFF & (v3 >> 16)]) << 16) | ((0xFF & sInv[0xFF & (v2 >> 8)]) << 8) | (0xFF & sInv[0xFF & v1]);
  var r1 =
      ((0xFF & sInv[0xFF & (v1 >> 24)]) << 24) | ((0xFF & sInv[0xFF & (v0 >> 16)]) << 16) | ((0xFF & sInv[0xFF & (v3 >> 8)]) << 8) | (0xFF & sInv[0xFF & v2]);
  var r2 =
      ((0xFF & sInv[0xFF & (v2 >> 24)]) << 24) | ((0xFF & sInv[0xFF & (v1 >> 16)]) << 16) | ((0xFF & sInv[0xFF & (v0 >> 8)]) << 8) | (0xFF & sInv[0xFF & v3]);
  var r3 =
      ((0xFF & sInv[0xFF & (v3 >> 24)]) << 24) | ((0xFF & sInv[0xFF & (v2 >> 16)]) << 16) | ((0xFF & sInv[0xFF & (v1 >> 8)]) << 8) | (0xFF & sInv[0xFF & v0]);
  r0 ^= key[keyIndex + 0];
  r1 ^= key[keyIndex + 1];
  r2 ^= key[keyIndex + 2];
  r3 ^= key[keyIndex + 3];
...
void aesEncryptBlock(
  ...
  for (var round = 1; round < rounds; round++) {
    ...
    final t1 = c0[0xFF & (v1 >> 24)] ^ c1[0xFF & (v2 >> 16)] ^ c2[0xFF & (v3 >> 8)] ^ c3[0xFF & (v0 >> 0)] ^ key[keyIndex + 1];
    final t2 = c0[0xFF & (v2 >> 24)] ^ c1[0xFF & (v3 >> 16)] ^ c2[0xFF & (v0 >> 8)] ^ c3[0xFF & (v1 >> 0)] ^ key[keyIndex + 2];
    final t3 = c0[0xFF & (v3 >> 24)] ^ c1[0xFF & (v0 >> 16)] ^ c2[0xFF & (v1 >> 8)] ^ c3[0xFF & (v2 >> 0)] ^ key[keyIndex + 3];
    v0 = t0;
    v1 = t1;
    v2 = t2;
    v3 = t3;
  ...
  // The last round with S-box constant or S-box inverse constant.
  const s = constants.s;
  var r0 = ((0xFF & s[0xFF & (v0 >> 24)]) << 24) | ((0xFF & s[0xFF & (v1 >> 16)]) << 16) | ((0xFF & s[0xFF & (v2 >> 8)]) << 8) | (0xFF & s[0xFF & v3]);
  var r1 = ((0xFF & s[0xFF & (v1 >> 24)]) << 24) | ((0xFF & s[0xFF & (v2 >> 16)]) << 16) | ((0xFF & s[0xFF & (v3 >> 8)]) << 8) | (0xFF & s[0xFF & v0]);
  var r2 = ((0xFF & s[0xFF & (v2 >> 24)]) << 24) | ((0xFF & s[0xFF & (v3 >> 16)]) << 16) | ((0xFF & s[0xFF & (v0 >> 8)]) << 8) | (0xFF & s[0xFF & v1]);
  var r3 = ((0xFF & s[0xFF & (v3 >> 24)]) << 24) | ((0xFF & s[0xFF & (v0 >> 16)]) << 16) | ((0xFF & s[0xFF & (v1 >> 8)]) << 8) | (0xFF & s[0xFF & v2]);
  r0 ^= key[keyIndex + 0];
  r1 ^= key[keyIndex + 1];
  r2 ^= key[keyIndex + 2];
  r3 ^= key[keyIndex + 3];
...
int _uint32ChangeEndian(int v) {
  // We mask with 0xFFFFFFFF to ensure the compiler recognizes the value will
  // be small enough to be a 'mint'.
  return (0xFFFFFFFF & ((0xFF & v) << 24)) | (0xFFFFFF & ((0xFF & (v >> 8)) << 16)) | (0xFFFF & ((0xFF & (v >> 16)) << 8)) | (0xFF & (v >> 24));
}
